...
static void	display_part(void) {
	...
	if (last_part == -2 && (g_bench || g_versus))
	{
		...
		printf("   _.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._.-=-._\n"
				".-'---      - ---     --     ---   -----   - --       ----  ----   -     ---`-.\n"
				" )  pts   time used by your libftmy      VS     %-25s pts  ( \n"COLOR_CLEAR, opponent);
	}
	...
	if (!old_fun_name || strcmp(old_fun_name, current_fun_name)) {
		...
		if (value == TEST_FINISHED)
		{
			if (g_bench != 0 || g_versus != NULL)
			{
				...
				winner = (total_player_points > total_versus_points) ? "WINNER: local libftmy" : "WINNER: system's libc";
			...
			{
				printf(COLOR_WARNING"\n\n\u26A0 : your libftmy is using forbidden functions so the result may not be relevant !\n"COLOR_CLEAR);
			}
			return ;
		}
		if (old_fun_name) {
